...
export default function MultiStepForm({ className, onSubmit }: MultiStepFormProps) {
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<z.infer<typeof currentStepSchema>>({
  ...
  // Handle next step
  const handleNextStep = (data: z.infer<typeof currentStepSchema>) => {
              ...
              <div key={s.id} className="flex flex-col items-center">
                <div
                  className={(() => {
                    ...
                    if (i === step) {
                      return cn(baseClasses, "bg-primary text-primary-foreground ring-2 ring-primary/30");
                    }
                ...
                {steps[step].fields.map((field) => (
                  <div key={field.name} className="space-y-2">
                    ...
                    <Input
                      ...
                      className={cn(errors[field.name as keyof FormData] && "border-destructive")}
                    />
